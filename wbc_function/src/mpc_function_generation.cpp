...
int main(){
    ...
    for(int i = 0; i < N; i++)
    {
        SX x_k_next = SX::zeros(state_length, 1); // x_(k+1) = A_k * x_k + B_k * u_k (k = 0, 1, 2, ..., N-1)
        SX u_k = SX::zeros(input_length, 1);  // u_k

        u_k = U(Slice(state_length * i, state_length * (i+1)));
        ...
        // dd = d * (1 + dT);
        dd = d * (1 + dT);
        x_k_next = mtimes(Ad, x_k) + mtimes(Bd, u_k) + dd;

        X(Slice(state_length * i, state_length * (i+1))) = x_k_next;

        x_k = x_k_next;
    }

    SX A_qp = jacobian(X, x0);
    SX B_qp = jacobian(X, U); 

    // COST FUNCTION
    SX W_Q = SX::sym("W_Q", N * state_length);  // TRAJECTORY TRACKING
    SX W_R = SX::sym("W_R", N * input_length);  // REGULATION
    SX J = mtimes(transpose(mtimes(A_qp, x0) + mtimes(B_qp, U) - X_ref), mtimes(diag(W_Q), mtimes(A_qp, x0) + mtimes(B_qp, U) - X_ref)) + mtimes(transpose(U - U_ref), mtimes(diag(W_R), U - U_ref));
    SX J_v = jacobian(J, U);
    SX J_vv = hessian(J, U);
    ...
    casadi::SX cineq2_min_v = jacobian(cineq2_min, U);
    casadi::SX cineq3_max_v = jacobian(cineq3_max, U);
    casadi::SX cineq3_min_v = jacobian(cineq3_min, U);
    casadi::SX cineq4_max_v = jacobian(cineq4_max, U);
    casadi::SX cineq4_min_v = jacobian(cineq4_min, U);
    casadi::SX cineq5_max_v = jacobian(cineq5_max, U);
    casadi::SX cineq5_min_v = jacobian(cineq5_min, U);

    // Generate CasADi functions
    Function J_v_func("J_v_func",   {x0, U, X_ref, W_Q, W_R, m, g, I, dT, rL_ref_horizon, rR_ref_horizon, theta_ref_horizon, etaL_ref_horizon, etaR_ref_horizon}, {J_v});
    Function J_vv_func("J_vv_func", {x0, U, X_ref, W_Q, W_R, m, g, I, dT, rL_ref_horizon, rR_ref_horizon, theta_ref_horizon, etaL_ref_horizon, etaR_ref_horizon}, {J_vv});

    Function X_func("X_func", {x0, U, m, g, I, dT, rL_ref_horizon, rR_ref_horizon, theta_ref_horizon, etaL_ref_horizon, etaR_ref_horizon}, {X});

    Function cineq1_max_func("cineq1_max_func", {U, f_z_max, f_z_min}, {cineq1_max});
    Function cineq1_min_func("cineq1_min_func", {U, f_z_max, f_z_min}, {cineq1_min});
    ...
    Function cineq1_max_v_func("cineq1_max_v_func", {U, f_z_max, f_z_min}, {cineq1_max_v});
    Function cineq1_min_v_func("cineq1_min_v_func", {U, f_z_max, f_z_min}, {cineq1_min_v});
