...
void KinWBC::computeKinematicWBC(
    ...
    const std::map<std::string, Eigen::Matrix3Vd>& base_Jacdot_v, const std::map<std::string, Eigen::Matrix3Vd>& base_Jacdot_w,
    ...
    for (const auto& task_group : task_hierarchy)
    {
        ...
        for (size_t i = 0; i < task_group.size(); ++i)
        {
            ...
            if (type == TaskType::Position)
            {
                ...
                Jdot.block(3 * i, 0, 3, dof_) = base_Jacdot_v.at(name);
                ...
                dde.segment<3>(3 * i) = ddx_desired.at(name)
                                      + Kp_vec.asDiagonal() * pos_err
                                      + Kv_vec.asDiagonal() * vel_err;
            }
            else if (type == TaskType::Orientation)
            {
                J.block(3 * i, 0, 3, dof_) = base_Jac_w.at(name);
                Jdot.block(3 * i, 0, 3, dof_) = base_Jacdot_w.at(name);
        ...
        // qddot_des += J_dyn_pinv * (dde - J * qddot_des - Jdot * qdot); 
